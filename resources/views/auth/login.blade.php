    ...
    <div class="min-h-screen flex">
        ...
        <div class="hidden lg:flex lg:w-1/2 bg-gradient-primary relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col justify-center px-12 text-light">
                ...
                <div class="mb-8">
                    <img src="{{ asset(config('app_branding.logos.full')) }}" alt="{{ config('app_branding.logos.alt') }}"
                        class="{{ config('app_branding.logos.sizes.login_desktop') }} mb-6">
                    <h1 class="text-4xl font-bold mb-4">{{ config('app_branding.auth.login.welcome_back') }}</h1>
                    <p class="text-xl text-light/90">
                        {{ str_replace('têxtil', config('app_branding.app.industry_adjective'), config('app_branding.auth.login.welcome_subtitle')) }}
                    </p>
                </div>
                ...
                <div class="bg-light/10 backdrop-blur-sm rounded-2xl p-6 max-w-md border border-light/20">
                    <div class="flex items-center mb-4">
                        <div class="w-12 h-12 bg-light/20 rounded-full flex items-center justify-center mr-4">
                            <span class="text-light font-bold">{{ config('app_branding.testimonial.initials') }}</span>
                        </div>
                        <div>
                            <h4 class="font-semibold">{{ config('app_branding.testimonial.name') }}</h4>
                            <p class="text-light/80 text-sm">{{ config('app_branding.testimonial.position') }},
                                {{ config('app_branding.testimonial.company') }}</p>
                        </div>
                    </div>
                    <p class="text-light/90 italic">
                        "O {{ config('app_branding.app.name') }} revolucionou nossa produção. Aumentamos
                        {{ config('app_branding.testimonial.efficiency_increase') }} nossa eficiência em apenas
                        {{ config('app_branding.testimonial.timeframe') }}."
                    </p>
                </div>
            </div>
            ...
            <div class="absolute inset-0 opacity-10">
                <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                        <pattern id="textile-pattern" x="0" y="0" width="60" height="60"
            ...
            <div class="w-full max-w-md mx-auto">
                ...
                <div class="lg:hidden text-center mb-8">
                    <img src="{{ asset(config('app_branding.logos.full')) }}"
                        ...
                        class="{{ config('app_branding.logos.sizes.login') }} mx-auto mb-4">
                </div>

                <!-- Header -->
                <div class="text-center mb-8">
                    <h2 class="text-3xl font-bold text-font mb-2">{{ config('app_branding.auth.login.title') }}</h2>
                    <p class="text-font/70">
                        {{ config('app_branding.auth.login.subtitle') }}
                        <a href="{{ route('register') }}"
                            class="text-primary hover:text-primary-hover font-semibold transition-colors">
                            {{ config('app_branding.cta.signup_link') }}
                        </a>
                    </p>
                </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-font/40" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <label for="remember_me" class="ml-2 block text-sm text-font">
                                    {{ config('app_branding.auth.login.remember_me') }}
                                </label>
                            </div>
                            ...
                            @if (Route::has('password.request'))
                                <div class="text-sm">
                                    <a href="{{ route('password.request') }}"
                                        class="text-primary hover:text-primary-hover font-semibold transition-colors">
                                        {{ config('app_branding.auth.login.forgot_password') }}
                                    </a>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="mt-8">
                        <div class="relative">
                            ...
                            <div class="relative flex justify-center text-sm">
                                <span
                                    class="px-2 bg-light text-font/50">{{ config('app_branding.auth.login.or_continue') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 grid grid-cols-2 gap-3">
                            <button
                                class="w-full inline-flex justify-center py-3 px-4 border border-font/20 rounded-xl shadow-sm bg-light text-sm font-medium text-font/50 hover:bg-bg transition-colors">
