        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input id="email" name="email" type="email"
                    class="mt-1 block w-full border-font/20 focus:border-primary focus:ring-primary rounded-md shadow-sm {{ !$user->isAdmin() ? 'bg-font/5' : 'bg-light' }} text-font"
                    value="{{ old('email', $user->email) }}" {{ $user->isAdmin() ? 'required' : 'readonly' }}
                    autocomplete="username" />
                @if (!$user->isAdmin())
                    <p class="mt-1 text-xs text-font/50">Apenas administradores podem alterar o e-mail</p>
                @endif
                <x-input-error class="mt-2" :messages="$errors->get('email')" />

                @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && !$user->hasVerifiedEmail())
                    <div class="mt-3 p-3 bg-warning/5 border border-warning/20 rounded-lg">
                        <p class="text-sm text-warning">
                            Seu e-mail ainda não foi verificado.
                            <button form="send-verification"
                                class="underline text-sm text-warning hover:text-warning-hover rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-warning/50">
        ...
        <div class="flex items-center gap-4">
            ...
            {{-- @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                    class="text-sm text-success flex items-center space-x-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
