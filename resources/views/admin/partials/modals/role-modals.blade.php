...
<div x-show="showCreateRoleModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-dark/50 transition-opacity" @click="closeCreateRoleModal()"></div>
        ...
        <div x-show="showCreateRoleModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <button @click="closeCreateRoleModal()" class="text-font/40 hover:text-font/60">
            ...
            <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-4">
                ...
                <!-- Nome -->
                <div>
                    <label for="create_role_name" class="block text-sm font-medium text-font mb-1">
                        Nome da Função *
                    </label>
                ...
                <!-- Descrição -->
                <div>
                    <label for="create_role_description" class="block text-sm font-medium text-font mb-1">
                        Descrição
                    </label>
                ...
                <!-- Ordem de Exibição -->
                <div>
                    <label for="create_role_sort_order" class="block text-sm font-medium text-font mb-1">
                        Ordem de Exibição
                    </label>
                    <input type="number" id="create_role_sort_order" name="sort_order" min="0"
                        class="w-full px-3 py-2 border border-font/20 rounded-lg focus:ring-primary focus:border-primary bg-light text-font"
                        placeholder="0 = primeiro, 999 = último">
                    <p class="text-xs text-font/50 mt-1">Menor número aparece primeiro na lista</p>
                </div>
...
<div x-show="showEditRoleModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-dark/50 transition-opacity" @click="closeEditRoleModal()"></div>
        ...
        <div x-show="showEditRoleModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <button @click="closeEditRoleModal()" class="text-font/40 hover:text-font/60">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <form :action="`/admin/roles/${editRoleId}`" method="POST" class="space-y-4">
                ...
                <!-- Nome -->
                <div>
                    <label for="edit_role_name" class="block text-sm font-medium text-font mb-1">
                        Nome da Função *
                    </label>
                ...
                <!-- Descrição -->
                <div>
                    <label for="edit_role_description" class="block text-sm font-medium text-font mb-1">
                        Descrição
                    </label>
                ...
                <!-- Ordem de Exibição -->
                <div>
                    <label for="edit_role_sort_order" class="block text-sm font-medium text-font mb-1">
                        Ordem de Exibição
                    </label>
                ...
                <!-- Status -->
                <div>
                    <label for="edit_role_is_active" class="block text-sm font-medium text-font mb-1">
                        Status *
                    </label>
...
<div x-show="showDeleteRoleModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-dark/50 transition-opacity" @click="closeDeleteRoleModal()"></div>
        ...
        <div x-show="showDeleteRoleModal" x-transition:enter="ease-out duration-300"
            ...
            <!-- Header -->
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-danger flex items-center space-x-2">
                ...
                <button @click="closeDeleteRoleModal()" class="text-font/40 hover:text-font/60">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
...
<div x-show="showRolePermissionsModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-dark/50 transition-opacity" @click="closePermissionsModal()"></div>
        ...
        <div x-show="showRolePermissionsModal" x-transition:enter="ease-out duration-300"
            ...
            x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
            x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
            class="inline-block align-bottom bg-light rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full sm:p-6">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <button @click="closePermissionsModal()" class="text-font/40 hover:text-font/60">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
            ...
            <!-- Role Info -->
            <div class="mb-6 p-4 bg-bg rounded-lg">
                <p class="text-sm text-font/70">
                    Configurando permissões para: 
                    <strong class="text-font" x-text="rolePermissionsName"></strong>
                </p>
            </div>
            ...
            <div x-show="loadingPermissions" class="text-center py-8">
                ...
                <p class="mt-2 text-sm text-font/60">Carregando permissões...</p>
            </div>
                ...
                <form @submit.prevent="savePermissions()" class="space-y-6">
                    ...
                    <div class="space-y-6">
                        <template x-for="(permissions, group) in permissionsData" :key="group">
                            <div class="border border-font/10 rounded-lg p-4">
                                <!-- Group Header -->
                                <div class="flex items-center justify-between mb-3">
                                    <h4 class="font-medium text-font" x-text="group || 'Geral'"></h4>
                                    <button type="button" 
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    <template x-for="permission in permissions" :key="permission.id">
                                        <label class="flex items-center space-x-2 p-2 rounded hover:bg-bg cursor-pointer">
                                            <input type="checkbox" 
                                                :value="permission.id"
                                                x-model="selectedPermissions"
                                                :data-group="group"
                                                class="rounded border-font/30 text-primary focus:ring-primary">
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-medium text-font" x-text="permission.name"></p>
                                                <p class="text-xs text-font/50" x-text="permission.description"></p>
                                            </div>
                                        </label>
                                    </template>
                                </div>
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-4 border-t border-font/10">
                        ...
                        <button type="submit" :disabled="savingPermissions"
                            class="px-4 py-2 text-sm font-medium text-light bg-primary border border-transparent rounded-lg hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50">
            ...
            <div x-show="!loadingPermissions && !permissionsData" class="text-center py-8" style="display: none;">
                <p class="text-danger">Erro ao carregar permissões</p>
            </div>
        </div>
    </div>
</div>
