...
<div x-show="showCreatePermissionModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="closeCreatePermissionModal()">
        </div>
        ...
        <div x-show="showCreatePermissionModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button @click="closeCreatePermissionModal()" class="text-gray-400 hover:text-gray-600">
            ...
            <form action="{{ route('admin.permissions.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <!-- Nome -->
                    <div>
                        <label for="create_permission_name" class="block text-sm font-medium text-gray-700">Nome da
                            Permissão</label>
                    ...
                    <!-- Slug -->
                    <div>
                        <label for="create_permission_slug" class="block text-sm font-medium text-gray-700">Slug</label>
                        <input type="text" id="create_permission_slug" name="slug"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-primary focus:border-primary"
                            placeholder="Ex: reports.view" pattern="[a-z0-9._-]+" required>
                        <p class="mt-1 text-xs text-gray-500">Use apenas letras minúsculas, números, pontos, hífens e
                            underscores</p>
                    </div>

                    <!-- Descrição -->
                    <div>
                        <label for="create_permission_description"
                            class="block text-sm font-medium text-gray-700">Descrição (opcional)</label>
                    ...
                    <div>
                        <label for="create_permission_group"
                            class="block text-sm font-medium text-gray-700">Grupo/Módulo</label>
                    ...
                    <!-- Ordem -->
                    <div>
                        <label for="create_permission_sort_order" class="block text-sm font-medium text-gray-700">Ordem
                            de Exibição</label>
                    ...
                    <div class="flex items-center">
                        ...
                        <label for="create_permission_is_active" class="ml-2 block text-sm text-gray-900">
                            Permissão ativa
                        </label>
                    </div>
                </div>
...
<div x-show="showEditPermissionModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="closeEditPermissionModal()">
        </div>
        ...
        <div x-show="showEditPermissionModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button @click="closeEditPermissionModal()" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <form :action="`/admin/permissions/${editPermissionId}`" method="POST">
                ...
                <div class="space-y-4">
                    <!-- Nome -->
                    <div>
                        <label for="edit_permission_name" class="block text-sm font-medium text-gray-700">Nome da
                            Permissão</label>
                    ...
                    <!-- Slug -->
                    <div>
                        <label for="edit_permission_slug" class="block text-sm font-medium text-gray-700">Slug</label>
                    ...
                    <!-- Descrição -->
                    <div>
                        <label for="edit_permission_description"
                            class="block text-sm font-medium text-gray-700">Descrição (opcional)</label>
                        <textarea id="edit_permission_description" name="description" rows="3" x-model="editPermissionDescription"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-primary focus:border-primary"></textarea>
                    </div>
                    ...
                    <!-- Ordem -->
                    <div>
                        <label for="edit_permission_sort_order" class="block text-sm font-medium text-gray-700">Ordem
                            de Exibição</label>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="edit_permission_is_active" name="is_active" value="1"
                            ...
                            class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
                        <label for="edit_permission_is_active" class="ml-2 block text-sm text-gray-900">
                            Permissão ativa
                        </label>
                    </div>
                </div>
...
<div x-show="showDeletePermissionModal" x-transition:enter="ease-out duration-300"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="closeDeletePermissionModal()">
        </div>
        ...
        <div x-show="showDeletePermissionModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button @click="closeDeletePermissionModal()" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-red-700">
                                <p>Você está prestes a excluir a permissão <strong
                                        x-text="deletePermissionName"></strong>.</p>
                                <p class="mt-1">Esta ação não pode ser desfeita e removerá a permissão de todas as
                                    funções que a possuem.</p>
                            </div>
                        </div>
                    </div>
                </div>
...
<div x-show="showNotificationModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="closeNotificationModal()">
        </div>
        ...
        <div x-show="showNotificationModal" x-transition:enter="ease-out duration-300"
            ...
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium flex items-center space-x-2"
                    ...
                    <!-- Success Icon -->
                    <svg x-show="notificationType === 'success'" class="w-5 h-5" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7">
                        </path>
                    </svg>
                    <!-- Error Icon -->
                    <svg x-show="notificationType === 'error'" class="w-5 h-5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
            ...
            <div class="flex items-center justify-end pt-4 border-t">
                <button @click="closeNotificationModal()"
                    ...
                    :class="notificationType === 'success' ? 'bg-green-600 hover:bg-green-700 focus:ring-green-500' :
